...
- **Избежание дубликатов**: проверка уже собранных постов
...
Для ежедневного автоматического обновления используйте scheduler:

```bash
python scheduler.py
```

По умолчанию обновление происходит каждый день в 9:00. Вы можете изменить время в файле `scheduler.py`.
...
# Добавить вечернее обновление в 18:00
...
- Не запускайте парсер слишком часто (рекомендуется 1-2 раза в день)
